...
void main() {
  ...
  for (int i = 0; i < numTests; i++) {
    ...
    log.info(
        'Test $i: Cross: ${crossPassed ? "PASS" : "FAIL"}, First Layer: ${firstLayerPassed ? "PASS" : "FAIL"}, Second Layer: ${secondLayerPassed ? "PASS" : "FAIL"}, Yellow Cross: ${yellowCrossPassed ? "PASS" : "FAIL"}');
...
bool _checkSecondLayer(CubeState s) {
  ...
  for (final f in middleFaces) {
    ...
    if (face[3] != face[4] || face[5] != face[4]) return false;
  }
  return true;
}

bool _checkYellowCross(CubeState s) {
  if (!_checkSecondLayer(s)) return false;
